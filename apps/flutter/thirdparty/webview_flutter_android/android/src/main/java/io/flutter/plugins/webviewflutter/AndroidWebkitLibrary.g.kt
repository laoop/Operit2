...
abstract class PigeonApiWebView(open val pigeonRegistrar: AndroidWebkitLibraryPigeonProxyApiRegistrar) {
  ...
  abstract fun getScale(pigeon_instance: android.webkit.WebView): Double

  /** Scales the current page viewport by a multiplicative factor. */
  abstract fun zoomBy(pigeon_instance: android.webkit.WebView, zoomFactor: Double)
  ...
  companion object {
    ...
    fun setUpMessageHandlers(binaryMessenger: BinaryMessenger, api: PigeonApiWebView?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.webview_flutter_android.WebView.getScale", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pigeon_instanceArg = args[0] as android.webkit.WebView
            val wrapped: List<Any?> = try {
              listOf(api.getScale(pigeon_instanceArg))
            } catch (exception: Throwable) {
              AndroidWebkitLibraryPigeonUtils.wrapError(exception)
            }
            reply.reply(wrapped)
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.webview_flutter_android.WebView.zoomBy", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pigeon_instanceArg = args[0] as android.webkit.WebView
            val zoomFactorArg = args[1] as Double
            val wrapped: List<Any?> = try {
              api.zoomBy(pigeon_instanceArg, zoomFactorArg)
              listOf(null)
            } catch (exception: Throwable) {
              AndroidWebkitLibraryPigeonUtils.wrapError(exception)
            }
